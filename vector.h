...
using iteratorB = Vector::IteratorB;
...
class Iterator{
	...
	//
	reference operator*(){
			/*if(ptr >= iterator.begin() && ptr < iterator.end())	
				return *ptr;
			else
				throw runtime_error("Pointer im unzulaessigen Bereich");*/
	...
	pointer operator->(){
 	...
 	/* bool operator==(const const_iterator& x){
		return static_cast<const_iterator>(x) == static_cast<const_iterator>(*this);
	}
...
/*class IteratorB{
	...
	friend bool operator==(const iteratorB& x, const iteratorB& c){
		return x.ptr == c.ptr;
	}
	...
	friend difference_type operator-(const iteratorB& lop, const iteratorB& rop){
		return lop.ptr-rop.ptr;
	}
...
class ConstIterator{
	...
	const reference operator*(){
			/*if(ptr >= Vector::begin() && ptr < Vector::end())	
				return *ptr;
			else
				throw runtime_error("Pointer im unzulaessigen Bereich");*/
...
iteratorB begin_b(const_reference limit) {return iterator(values);}
iteratorB end_b() {return iterator(values+sz);}
